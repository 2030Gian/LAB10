...
async def get_pokemon_image(pokemon_name: str) -> dict[str, Any]:
    api = f"/images/{pokemon_name}"
    function = "get_pokemon_image"
    start_time = time.perf_counter()

    pokemon_name = pokemon_name.strip().lower()
    url = f"{settings.POKE_IMAGES_BASE_URL.rstrip('/')}/images/{pokemon_name}"
    ...
    try:
        ...
        if response.status_code == 404:
            ...
            raise ValueError(f"Image for Pokemon '{pokemon_name}' not found")
        ...
        raise RuntimeError("Error calling Poke Images Service")
